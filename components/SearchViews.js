import React, { Component } from 'react';
import Title from './SearchTitle.js';

class SearchViews extends Component {

  handleSearchViewActive = (e) => {
    const searchViewItem = document.getElementById("searchViews-item")
    searchViewItem.classList.toggle('active')

    const clear = document.getElementById("clearSearchView")
    clear.classList.add('active')
  }

  handleSearchViewActive2 = (e) => {
    const searchViewItem = document.getElementById("searchViews-item2")
    searchViewItem.classList.toggle('active')

    const clear = document.getElementById("clearSearchView")
    clear.classList.add('active')
  }

  handleSearchViewActive3 = (e) => {
    const searchViewItem = document.getElementById("searchViews-item3")
    searchViewItem.classList.toggle('active')

    const clear = document.getElementById("clearSearchView")
    clear.classList.add('active')
  }

  handleClear = e => {

    const searchViewItem = document.getElementsByClassName("searchViews-item")

    for(var i=0; i<searchViewItem.length; i++) {
      if(searchViewItem[i].classList.contains('active')) {
        searchViewItem[i].classList.remove('active')
      }
    }

    const clear = document.getElementById("clearSearchView")
    clear.classList.remove('active')
  }

  render() {

    return (
      <div className="searchViews">
        <Title title="Views"></Title>
        <ul className="searchViews-items" id="SearchMaterialType">
            <li className="searchViews-item" id="searchViews-item" onClick={this.handleSearchViewActive}>
                <a >
                  <div className="icon mia">
                    <svg className="trendingIcon" style={{width:50, borderRadius: 0, height: 70}} version="1.1"  viewBox="0 0 512 640">
                      <g>
                        <path className="tw" d="M446.73,268.439h-45.29c-3.741,0-6.774,3.033-6.774,6.774s3.033,6.774,6.774,6.774h38.515v180.271h-31.74V305.661   c0-3.741-3.033-6.774-6.774-6.774s-6.774,3.033-6.774,6.774v163.372c0,3.741,3.033,6.773,6.774,6.773h45.29   c3.74,0,6.773-3.032,6.773-6.773V275.214C453.504,271.473,450.471,268.439,446.73,268.439z"/>
                        <path className="tw" d="M361.641,352.701h-45.289c-3.742,0-6.775,3.033-6.775,6.774v109.558c0,3.741,3.033,6.773,6.775,6.773h45.289   c3.741,0,6.773-3.032,6.773-6.773V359.476C368.414,355.734,365.382,352.701,361.641,352.701z M354.866,462.259h-31.741V366.25   h31.741V462.259z"/>
                        <path className="tw" d="M276.551,390.57h-45.289c-3.742,0-6.774,3.033-6.774,6.775v71.688c0,3.741,3.033,6.773,6.774,6.773h45.289   c3.742,0,6.774-3.032,6.774-6.773v-71.688C283.325,393.604,280.293,390.57,276.551,390.57z M269.776,462.259h-31.741v-58.14h31.741   V462.259z"/>
                        <path className="tw" d="M191.461,422.544h-45.29c-3.741,0-6.774,3.032-6.774,6.774v39.715c0,3.741,3.033,6.773,6.774,6.773h45.29   c3.741,0,6.774-3.032,6.774-6.773v-39.715C198.235,425.576,195.203,422.544,191.461,422.544z M184.687,462.259h-31.741v-26.166   h31.741V462.259z"/>
                        <path className="tw" d="M413.077,177.319c1.143-2.308,0.877-5.065-0.685-7.113l-26.73-35.032c-1.281-1.68-3.273-2.666-5.385-2.666   c-2.113,0-4.105,0.986-5.387,2.666l-26.729,35.032c-2.27,2.974-1.697,7.225,1.276,9.495c1.228,0.936,2.672,1.389,4.104,1.389   c2.041,0,4.057-0.917,5.391-2.665l21.344-27.974l13.039,17.09h-13.039c-3.742,0-6.774,3.033-6.774,6.774   c0,97.535-79.351,176.885-176.885,176.885c-3.742,0-6.774,3.033-6.774,6.775c0,3.74,3.033,6.773,6.774,6.773   c102.737,0,186.718-81.78,190.3-183.659h20.088C409.582,181.089,411.936,179.628,413.077,177.319z"/>
                        <path className="tw" d="M289.732,276.23c0.386-0.295,0.733-0.629,1.048-0.999c27.02-25.228,43.958-61.123,43.958-100.917   c0-76.16-61.961-138.121-138.121-138.121c-3.452,0-6.862,0.17-10.251,0.42c-0.008,0-0.016-0.001-0.024,0   c-0.005,0-0.01,0.002-0.016,0.003c-38.972,2.887-73.493,21.985-96.806,50.595c-0.391,0.383-0.723,0.819-1.013,1.288   c-18.762,23.587-30.013,53.403-30.013,85.814c0,36.062,13.906,68.926,36.62,93.55c0.29,0.378,0.619,0.72,0.981,1.029   c25.207,26.774,60.933,43.542,100.521,43.542C232.463,312.436,265.157,298.703,289.732,276.23z M191.14,274.525   c-14.631-11.31-32.874-17.537-51.368-17.537h-36.23c-19.574-22.013-31.497-50.971-31.497-82.674   c0-28.364,9.547-54.528,25.571-75.492h37.786c9.966,0,18.075,8.108,18.075,18.075c0,9.967-8.109,18.076-18.075,18.076h-18.369   c-15.455,0-28.029,12.574-28.029,28.029c0,15.456,12.574,28.029,28.029,28.029h1.417c9.157,0,16.605,7.45,16.605,16.607   c0,9.157-7.449,16.606-16.605,16.606c-3.742,0-6.774,3.032-6.774,6.774c0,3.741,3.033,6.774,6.774,6.774   c16.627,0,30.154-13.528,30.154-30.155c0-16.628-13.527-30.155-30.154-30.155h-1.417c-7.985,0-14.48-6.496-14.48-14.481   c0-7.985,6.496-14.48,14.48-14.48h18.369c17.437,0,31.624-14.186,31.624-31.625c0-17.438-14.187-31.624-31.624-31.624h-25.785   c19.031-18.599,43.973-31.148,71.714-34.561c5.422,24.208,26.823,41.732,52.294,41.732h19.699c3.741,0,6.773-3.033,6.773-6.774   s-3.032-6.774-6.773-6.774h-19.699c-18.304,0-33.787-12.107-38.555-29.116c0.518-0.006,1.028-0.039,1.547-0.039   c68.689,0,124.572,55.883,124.572,124.573c0,35.244-14.736,67.092-38.342,89.774h-22.665c-7.25,0-13.148-5.897-13.148-13.147   s5.898-13.147,13.148-13.147h2.568c15.05,0,27.294-12.245,27.294-27.294s-12.244-27.293-27.294-27.293h-24.105   c-11.336,0-20.558-9.223-20.558-20.559s9.222-20.558,20.558-20.558h49.802c3.741,0,6.773-3.032,6.773-6.774   s-3.032-6.774-6.773-6.774h-49.802c-18.807,0-34.106,15.3-34.106,34.106s15.3,34.107,34.106,34.107h24.105   c7.579,0,13.745,6.166,13.745,13.745s-6.166,13.745-13.745,13.745h-2.568c-14.721,0-26.696,11.976-26.696,26.696   s11.976,26.695,26.696,26.695h5.951c-19.869,13.412-43.794,21.25-69.516,21.25c-29.976,0-57.51-10.646-79.028-28.351h22.182   c15.511,0,30.811,5.224,43.083,14.708c1.232,0.953,2.691,1.414,4.138,1.414c2.025,0,4.03-0.904,5.364-2.632   C194.645,281.066,194.101,276.813,191.14,274.525z"/>
                      </g>
                    </svg>
                  </div>
                  <h4>Trending</h4>
                </a>
            </li>
            <li className="searchViews-item" id="searchViews-item2" onClick={this.handleSearchViewActive2}>
                <a >
                  <div className="icon mia">
                    <svg className="mostViewIcon" style={{width:40, borderRadius: 0, height: 64}} height="512pt" viewBox="0 -20 512 512" width="512pt" xmlns="http://www.w3.org/2000/svg"><path d="m256.238281 368c-4.421875 0-8.039062-3.574219-8.039062-8s3.535156-8 7.953125-8h.085937c4.417969 0 8 3.574219 8 8s-3.582031 8-8 8zm0 0"/><path d="m504 352h-495.625c-4.421875 0-8-3.574219-8-8s3.578125-8 8-8h495.625c4.425781 0 8 3.574219 8 8s-3.574219 8-8 8zm0 0"/><path d="m488 384h-464c-13.230469 0-24-10.769531-24-24v-336c0-13.230469 10.769531-24 24-24h464c13.230469 0 24 10.769531 24 24v336c0 13.230469-10.769531 24-24 24zm-464-368c-4.414062 0-8 3.585938-8 8v336c0 4.414062 3.585938 8 8 8h464c4.414062 0 8-3.585938 8-8v-336c0-4.414062-3.585938-8-8-8zm0 0"/><path d="m184.007812 448c-.640624 0-1.296874-.078125-1.945312-.238281-4.285156-1.074219-6.894531-5.417969-5.824219-9.699219l16-64c1.066407-4.292969 5.417969-6.917969 9.699219-5.824219 4.285156 1.074219 6.894531 5.417969 5.824219 9.699219l-16 64c-.914063 3.636719-4.167969 6.0625-7.753907 6.0625zm0 0"/><path d="m327.992188 448c-3.585938 0-6.839844-2.425781-7.753907-6.0625l-16-64c-1.078125-4.28125 1.53125-8.625 5.824219-9.699219 4.273438-1.101562 8.625 1.53125 9.699219 5.824219l16 64c1.078125 4.28125-1.53125 8.625-5.824219 9.699219-.648438.160156-1.304688.238281-1.945312.238281zm0 0"/><path d="m152 464c-4.425781 0-8-3.574219-8-8 0-13.230469 10.769531-24 24-24 4.425781 0 8 3.574219 8 8s-3.574219 8-8 8c-4.414062 0-8 3.585938-8 8 0 4.425781-3.574219 8-8 8zm0 0"/><path d="m360 464c-4.425781 0-8-3.574219-8-8 0-4.414062-3.585938-8-8-8-4.425781 0-8-3.574219-8-8s3.574219-8 8-8c13.230469 0 24 10.769531 24 24 0 4.425781-3.574219 8-8 8zm0 0"/><path d="m344 448h-176c-4.425781 0-8-3.574219-8-8s3.574219-8 8-8h176c4.425781 0 8 3.574219 8 8s-3.574219 8-8 8zm0 0"/><path d="m152 472c-4.425781 0-8-3.574219-8-8v-8c0-4.425781 3.574219-8 8-8s8 3.574219 8 8v8c0 4.425781-3.574219 8-8 8zm0 0"/><path d="m360 472h-208c-4.425781 0-8-3.574219-8-8s3.574219-8 8-8h208c4.425781 0 8 3.574219 8 8s-3.574219 8-8 8zm0 0"/><path d="m360 472c-4.425781 0-8-3.574219-8-8v-8c0-4.425781 3.574219-8 8-8s8 3.574219 8 8v8c0 4.425781-3.574219 8-8 8zm0 0"/><path d="m256 296c-73.777344 0-143.289062-42.160156-190.71875-115.664062-1.714844-2.640626-1.714844-6.03125 0-8.671876 47.429688-73.503906 116.941406-115.664062 190.71875-115.664062s143.289062 42.160156 190.71875 115.664062c1.714844 2.640626 1.714844 6.03125 0 8.671876-47.429688 73.503906-116.941406 115.664062-190.71875 115.664062zm-174.433594-120c44.320313 66.167969 107.640625 104 174.433594 104s130.113281-37.832031 174.433594-104c-44.320313-66.167969-107.640625-104-174.433594-104s-130.113281 37.832031-174.433594 104zm0 0"/><path d="m256 264c-48.519531 0-88-39.480469-88-88s39.480469-88 88-88 88 39.480469 88 88-39.480469 88-88 88zm0-160c-39.703125 0-72 32.296875-72 72s32.296875 72 72 72 72-32.296875 72-72-32.296875-72-72-72zm0 0"/><path d="m256 208c-17.648438 0-32-14.351562-32-32s14.351562-32 32-32 32 14.351562 32 32-14.351562 32-32 32zm0-48c-8.824219 0-16 7.175781-16 16s7.175781 16 16 16 16-7.175781 16-16-7.175781-16-16-16zm0 0"/></svg>
                  </div>
                  <h4>Most Views</h4>
                </a>
            </li>
            <li className="searchViews-item" id="searchViews-item3" onClick={this.handleSearchViewActive3}>
                <a>
                  <div className="icon mia">
                    <svg className="latestIcon" style={{width:37, borderRadius: 0, height: 60}} viewBox="0 0 512.017 512.017">
                      <g transform="translate(0 -1)">
                        <g>
                          <g>
                            <path d="M392.512,128.748v-3.081L276.828,1.017h-3.182c-0.118-0.007-0.235,0.002-0.353,0H0.038v512h392.474V129.175
                              C392.516,129.033,392.516,128.891,392.512,128.748z M364.415,120.484h-82.779V31.29L364.415,120.484z M17.105,495.951V18.084
                              H264.57v110.933c0,4.713,3.82,8.533,8.533,8.533h102.342v358.4H17.105z"/>
                            <path d="M341.312,222.884h-128c-4.713,0-8.533,3.82-8.533,8.533s3.821,8.533,8.533,8.533h128c4.713,0,8.533-3.82,8.533-8.533
                              S346.025,222.884,341.312,222.884z"/>
                            <path d="M213.312,205.817h110.933c4.713,0,8.533-3.82,8.533-8.533s-3.82-8.533-8.533-8.533H213.312
                              c-4.713,0-8.533,3.82-8.533,8.533S208.599,205.817,213.312,205.817z"/>
                            <path d="M141.566,190.451c-3.773-2.823-9.121-2.053-11.945,1.72l-19.639,26.247l-10.102-10.102
                              c-3.332-3.332-8.735-3.332-12.068,0s-3.332,8.735,0,12.068l17.067,17.067c3.675,3.675,9.753,3.239,12.866-0.922l25.54-34.133
                              C146.11,198.623,145.34,193.275,141.566,190.451z"/>
                            <path d="M110.912,146.084c-37.703,0-68.267,30.564-68.267,68.267c0,37.703,30.564,68.267,68.267,68.267
                              s68.267-30.564,68.267-68.267C179.179,176.648,148.615,146.084,110.912,146.084z M110.912,265.551
                              c-28.277,0-51.2-22.923-51.2-51.2c0-28.277,22.923-51.2,51.2-51.2s51.2,22.923,51.2,51.2
                              C162.112,242.628,139.189,265.551,110.912,265.551z"/>
                            <path d="M341.312,402.084h-128c-4.713,0-8.533,3.82-8.533,8.533s3.821,8.533,8.533,8.533h128c4.713,0,8.533-3.82,8.533-8.533
                              S346.025,402.084,341.312,402.084z"/>
                            <path d="M213.312,385.017h110.933c4.713,0,8.533-3.82,8.533-8.533s-3.82-8.533-8.533-8.533H213.312
                              c-4.713,0-8.533,3.82-8.533,8.533S208.599,385.017,213.312,385.017z"/>
                            <path d="M141.566,369.651c-3.773-2.823-9.121-2.053-11.945,1.72l-19.639,26.247l-10.102-10.102
                              c-3.332-3.332-8.735-3.332-12.068,0s-3.332,8.735,0,12.068l17.067,17.067c3.675,3.675,9.753,3.239,12.866-0.922l25.54-34.133
                              C146.11,377.823,145.34,372.475,141.566,369.651z"/>
                            <path d="M110.912,325.284c-37.703,0-68.267,30.564-68.267,68.267c0,37.703,30.564,68.267,68.267,68.267
                              s68.267-30.564,68.267-68.267C179.179,355.848,148.615,325.284,110.912,325.284z M110.912,444.751
                              c-28.277,0-51.2-22.923-51.2-51.2c0-28.277,22.923-51.2,51.2-51.2s51.2,22.923,51.2,51.2
                              C162.112,421.828,139.189,444.751,110.912,444.751z"/>
                            <path d="M486.507,1.017h-34.389c-14.065,0-25.472,11.407-25.472,25.472v34.261v51.2v318l34.133,59.733v14.8
                              c0,4.713,3.82,8.533,8.533,8.533s8.533-3.82,8.533-8.533v-14.801l34.133-59.733v-318v-51.2V26.489
                              C511.979,12.424,500.572,1.017,486.507,1.017z M443.712,77.817h51.2v17.067h-51.2V77.817z M469.312,470.218l-19.429-34.001
                              h38.858L469.312,470.218z M494.912,419.151h-51.2v-307.2h51.2V419.151z M443.712,60.751V26.489c0-4.64,3.766-8.405,8.405-8.405
                              h34.389c4.64,0,8.405,3.766,8.405,8.405v34.261H443.712z"/>
                          </g>
                        </g>
                      </g>
                      <g>
                      </g>
                      <g>
                      </g>
                      <g>
                      </g>
                      <g>
                      </g>
                      <g>
                      </g>
                      <g>
                      </g>
                      <g>
                      </g>
                      <g>
                      </g>
                      <g>
                      </g>
                      <g>
                      </g>
                      <g>
                      </g>
                      <g>
                      </g>
                      <g>
                      </g>
                      <g>
                      </g>
                      <g>
                      </g>
                      </svg>
                  </div>
                  <h4>Latest</h4>
                </a>
            </li>
        </ul>
        
        <span className="clearSearchView" id="clearSearchView" onClick={this.handleClear}><i>&#x2715; </i> Clear selection </span>

        <style>{`
          .searchViews {
            background: #e1e2e4;
            box-sizing: border-box;
            font-family: sans-serif;
            border-radius: 5px;
            margin-bottom: 10px;
            text-align: center;
          }

          .searchViews-items {
            display: flex;
            justify-content: space-around;
            align-items: center;
            flex-wrap: wrap;
            text-align: center;
          }

          .searchViews-item a {
            text-decoration: none;
          }

          .searchViews-item {
            margin: 10px 10px 20px 10px;
            cursor: pointer;
          }

          .searchViews-item:hover h4:before,
          .searchViews-item.active h4:before{
            width: 100%;
          }

          .searchViews-item:hover .icon,
          .searchViews-item.active .icon{
            border: 2px solid #ffa603;
            background: #1c2d4c;
          }

          .searchViews-item:hover svg,
          .searchViews-item.active svg{
            fill: #ffa603;
          }

          .searchViews-item path,
          .searchViews-item path{
            stroke-dasharray: 300, 0, 3000;
          }

          .searchViews-item:hover path,
          .searchViews-item.active path{
            animation: twitter 2s;
          }

          .searchViews-item h4 {
            margin-top: 10px;
            color: #1c2d4c;
            border-bottom: 2px solid transparent;
            transition: .25s linear;
            position: relative;
            display: inline-block;
          }

          .searchViews-item h4:before {
            content: "";
            position: absolute;
            bottom: -5px;
            left: 0;
            height: 2px;
            width: 0;
            background: #1c2d4c;
            transition: .15s linear;
          }

          .searchViews-item svg{
            fill: #1c2d4c;
            margin: 0 auto;
            width: 70px;
            height: 70px;
            background: transparent;
            border-radius: 50%;
            transition: .25s linear;
          }

          .icon {
            width: 70px;
            height: 65px;
            background: transparent;
            padding-top: 5px;
            border-radius: 50%;
            transition: .25s linear;
            border: 2px solid #1c2d4c;
          }

          .clearSearchView {
            margin: 10px auto;
            display: none;
            text-align: center;
            cursor: pointer;
            transition: .25s linear;
            padding: 5px;
            border: 1px solid transparent;
            font-weight: 100;
          }

          .clearSearchView:hover {
            background: #1c2d4c;
            border-radius: 4px;
            color: orange;
            border: 1px solid orange;
          }

          .clearSearchView.active {
            display: inline-block;
          }

          @media screen and (max-width: 767px) {
            

            .searchViews-item .trendingIcon {
              height: 60px !important;
            }

            .searchViews-item .mostViewIcon {
              height: 55px !important;
            }

            .searchViews-item .latestIcon {
              height: 50px !important;
            }
          }

        `}</style>

      </div>
    );
  }
}

export default SearchViews;
